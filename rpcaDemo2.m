...
start_up
clc
dataset='C:\Users\Administrator\Documents\MATLAB\matrixfactorization_xcde\dataset\ORL_32x32.mat';
load(dataset)
method_name='RPCA';
faceW = 32; 
faceH = 32; 
...
% opts1.cols=size(X,2);
load('rpca_list.mat');
listlength=size(rpcalist,2);
for i=1:listlength
% algorithm_id1='GA';
    for j=1:listlength
    algorithm_id1=rpcalist{i};
    % algorithm_id1='FPCP';
    algorithm_id2=rpcalist{j};
    % algorithm_id2='PCP';
...
%     imshow(Img);
    ...
    % mkdir(filename);
    saveas(gcf,filename,fileformat1);
    % saveas(h_fig,filename,fileformat2);

    displog([algorithm_id1 '_' algorithm_id2 '--end']);
    end
end
