...
load(dataset1)
load('a_mc_decomposeData.mat')
method_name='MC';

algorithmlist=mc_decompose_list

algorithm_start=1
algorithm_end=length(algorithmlist)
mc_clustering_list=[]
 save('a_mc_clusteringData','mc_clustering_list')
for i=algorithm_start:algorithm_end
    ...
    mc_clustering_list=[mc_clustering_list; B]
    save('a_mc_clusteringData','mc_clustering_list', '-append')
end
  data=load('a_mc_clusteringData');
 f=fieldnames(data);
 for k=1:size(f,1)
   xlswrite('a_mc_clusteringData.xlsx',data.(f{k}),f{k})
 end